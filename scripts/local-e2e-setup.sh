...
suggest_dev_vault_if_missing() {
  ...
  if [[ "${have_env_creds}" == "true" ]]; then
    ...
    # Let dev-vault auto-detect OpenShift and import images with a short timeout.
...
ensure_argocd_ui_rbac() {
  ...
  if oc -n openshift-gitops get argocd openshift-gitops >/dev/null 2>&1; then
    # Grant Argo CD admin to cluster-admins group and kubeadmin principal.
    # Casbin uses 'g' for subject→role mappings; there is no 'u' directive.
